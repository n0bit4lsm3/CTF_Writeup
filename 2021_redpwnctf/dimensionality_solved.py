...
for i in range(28):
	s.add(Or(inp[i] == ord('b'), inp[i] == ord('d'), inp[i] == ord('f'), inp[i] == ord('l'), inp[i] == ord('r'), inp[i] == ord('u')))

for i in range(28):
	a3 += If(inp[i] == ord('b'), func_b(), If(inp[i] == ord('d'), func_d(), If(inp[i] == ord('f'), func_f(), If(inp[i] == ord('l'),
		func_l(), If(inp[i] == ord('r'), func_r(), If(inp[i] == ord('u'), func_u(), func_default()))))))

	s.add(a3 > 0, a3 < 0xb * 0xb * 0xb)
...
while s.check() == sat:
    ...
    r = process('./dimensionality', level='error')
    r.sendline(w)
    n = r.recv()
    if b'flag' in n:
        print(w)
